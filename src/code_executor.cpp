...
OutputResult CodeExecutor::execute(const InputStruct& input) {
    ...
    if (compileResult != Verdict::Accepted) {
        ...
        errorResult.stderr_data = compileError;
...
TestCaseResult CodeExecutor::executeInSandbox(const std::string& execPath,
    ...
    result.stdin_data = testCase.stdin_data;
    ...
    if (pid == 0) {
        ...
        // 写入输入数据
        write(stdin_pipe[1], testCase.stdin_data.c_str(), testCase.stdin_data.length());
        close(stdin_pipe[1]);
        ...
        while ((n = read(stdout_pipe[0], buffer, sizeof(buffer))) > 0) {
            ...
            if (result.stdout_data.length() > (size_t)limits.output_bytes) {
                kill(pid, SIGKILL);
                result.status = Verdict::RuntimeError;
                result.stderr_data = "Output limit exceeded";
                break;
            }
        }
        close(stdout_pipe[0]);
        ...
        if (WIFEXITED(status)) {
            ...
            if (exitCode == 0) {
                // 比较输出
                if (compareOutput(result.stdout_data, testCase.expected)) {
